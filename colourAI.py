...
# labels = ['wRook', 'wQueen','wPawn','wKnight','wKing','wBishop','empty','bRook','bQueen','bPawn','bKnight','bKing','bBishop']
...
history = model.fit(x_train, y_train, batch_size=500, epochs=50, shuffle = True, validation_split=0.01,class_weight=dict(zip(np.unique(y_train), class_weight.compute_class_weight('balanced', np.unique(y_train), 
                y_train))) )

model.save("colourClassify")

while True:

    choice = np.random.randint(0,21000)

    convertor = (dict(enumerate(labels))) 

    model = keras.models.load_model("colourClassify")
